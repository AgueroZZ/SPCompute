...
#' @return The power that can be ahieved at the given sample size.
#' @examples
#' parameters <- list(TraitMean = 0.3, TraitSD = 1, pG = 0.2, betaG = log(1.1), betaE = log(1.1), muE = 0, sigmaE = 3, gammaG = log(2.1))
#' Compute_Power_Sim(parameters, n = 1000, B = 10000, "continuous", "continuous")
Compute_Power_Sim <- function(parameters, n, B = 10000, response = "binary", covariate = "binary", mode = "additive", alpha = 0.05, seed = 123, LargePowerApproxi = F){
  ...
  if(response == "binary"){
    ...
    if(covariate == "continuous"){
      ...
      if(mode == "additive"){
        ...
        I[2,3] <- EGE
        I[3,3] <- EE2
        I <- Matrix::forceSymmetric((SigmaError1^2)* I, uplo = "U")
        if(LargePowerApproxi){
          SE <- sqrt((solve(I)[2,2]))/sqrt(n)
          return(pnorm(-qnorm(1-(alpha/2)) + betaG/SE))
        }
      ...
      if(mode == "additive"){
        ...
        gamma0 <- uniroot(FunctionForGamma0, c(-8, 8))$root
...
Compute_Size_Sim <- function(parameters, PowerAim, B = 10000, response = "binary", covariate = "binary", mode = "additive", alpha = 0.05, seed = 123, LargePowerApproxi = F, upper.lim.n = 800000){
  if(check_parameters(parameters, response, covariate) != T){return(message("Define the above missing parameters before continuing"))}
  if(response == "binary"){
    if(covariate == "binary"){
      ### Compute probability
    ...
    if(covariate == "continuous"){
      ...
      if(mode == "additive"){
        ...
        if(LargePowerApproxi){
          ...
          uniroot(compute_power_diff, c(0, 800000))$root
        }
      }
      else if(mode == "dominant"){
        qG <- qG^2
...
Compute_Power_Expanded <- function(parameters, n, response = "binary", covariate = "binary", mode = "additive", alpha = 0.05, seed = 123, LargePowerApproxi = F){
  if(check_parameters(parameters, response, covariate) != T){return(message("Define the above missing parameters before continuing"))}
  if(response == "binary"){
    if(covariate == "binary"){
      ### Compute probability
...
Compute_Size_Expanded <- function(parameters, PowerAim, response, covariate, mode, alpha = 0.05, seed = 123, LargePowerApproxi = F, upper.lim.n = 800000){
  if(check_parameters(parameters, response, covariate) != T){return(message("Define the above missing parameters before continuing"))}
  compute_power_diff <- function(n){
    ### Once know this SE of betaG hat, compute its power at this given sample size n:
    Power = Compute_Power_Expanded(parameters, n, response = response, covariate = covariate, mode = mode, alpha = alpha, seed = seed, LargePowerApproxi = LargePowerApproxi)
    Power - PowerAim
  }
  if(compute_power_diff(100) >= 0) return(message("The required sample size is estimated to be less than 100."))
  if(compute_power_diff(800000) <=0) return(message(paste("The required sample size is estimated to be larger than upper.lim.n, which is", upper.lim.n, sep = " ")))
  uniroot(compute_power_diff, c(101, upper.lim.n))$root
}
...
Compute_Size_Sim_simpler <- function(parameters, PowerAim, B = 10000, response = "binary", covariate = "binary", mode = "additive", alpha = 0.05, seed = 123, LargePowerApproxi = F, upper.lim.n = 800000){
  ...
  if(compute_power_diff(upper.lim.n) <=0) return(message(paste("The required sample size is estimated to be larger than upper.lim.n, which is", upper.lim.n, sep = " ")))
  uniroot(compute_power_diff, c(1, upper.lim.n))$root
}
